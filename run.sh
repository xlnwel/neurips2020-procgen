...
#   export LATEST_CHECKPOINT_DIRECTORY=$(ls -trd ${LATEST_EXECUTION_DIRECTORY%/}/*/ | tail -n 1)
  export LATEST_EXECUTION_DIRECTORY=~/ray_results/agent-bigfish/Agent_my_procgen_0_2021-01-04_22-37-20529ueorg
...
if [[ " $1 " =~ " --rollout " ]]; then
  ...
  python ./rollout.py $CHECKPOINT --episodes ${EPISODES:-100} --run $ROLLOUT_RUN --env $ROLLOUT_ENV -n 3 # --video-dir ~/ray_results/video
  STATUS_CODE=$?
fi
