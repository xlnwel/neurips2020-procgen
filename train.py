...
def run(args, parser):
    ...
    for k, v in experiments.items():
        experiments[k].pop('disable_evaluation_worker', False)
    ...
    from ray.tune.tune import run as run_exp
    ...
    run_exp(
        ...
        resume=args.resume,
        trial_name_creator=lambda: args.name,
        verbose=verbose,)

def save_code(root_dir, model_name=''):
    import shutil

    dest_dir = f'{root_dir}/src/{model_name}' if model_name else root_dir
...
if __name__ == "__main__":
    ...
    dest_dir = save_code(args.local_dir, args.name)
    run(args, parser)
    print(dest_dir)