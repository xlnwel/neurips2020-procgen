...
from calculator import reward2score
from envs.wrappers import get_wrapper_by_name
...
def rollout(agent,
    ...
    els = []
    start = time.time()
    env_name = get_wrapper_by_name(env, 'ProcgenEnvWrapper').env_name
    while keep_going(steps, num_steps, episodes, num_episodes):
        ...
        if done:
            episodes += 1
        rs.append(reward_total)
        els.append(episode_steps)
        print("Episode #{}: reward: {}-{:.3g} steps: {}-{:.3g} score: {:.3g}".format(
            episodes, reward_total, np.mean(rs), episode_steps, np.mean(els),
            reward2score(env_name, np.mean(rs))))
    duration = time.time() - start
    score = reward2score(env_name, np.mean(rs))
    print(f'Episode rewards: mean({np.mean(rs):.3g}), std({np.std(rs):.3g})')
    print(f'Episode lengths: mean({np.mean(els):.3g}), std({np.std(els):.3g})')
    print(f'Duration: {duration:.3g}')
    print(f'{env_name} score: {score:.3g}')
...
if __name__ == "__main__":
    ...
    start = time.time()
    reward_list, episode_length_list, score_list = [], [], []
    for _ in range(args.num_trials):
        rs, els, score = run(args, parser)
        reward_list += rs
        episode_length_list += els
        score_list.append(score)
    duration = time.time() - start
    print(f'Episode rewards: mean({np.mean(reward_list):.3g}), std({np.std(reward_list):.3g})')
    print(f'Episode lengths: mean({np.mean(episode_length_list):.3g}), std({np.std(episode_length_list):.3g})')
    print(f'Duration: {duration:.3g}')
    print(f'Reward: mean({np.mean(reward_list):.3g}), std({np.std(reward_list):.3g})')
    print(f'Score: mean({np.mean(score_list):.3g}), std({np.std(score_list):.3g})')