...
from .policy import data_augmentation
...
class ReplayBuffer:
    ...
    def apply_da(self, x):
        ...
        x = np.pad(x, paddings, mode=self._da['mode'])
    ...
    @DeveloperAPI
    def add(self, obs_t, action, reward, obs_tp1, done, weight, step, obs_da):
        if obs_da is None:
            data = (obs_t, action, reward, obs_tp1, done, step)
        else:
            data = (obs_t, action, reward, obs_tp1, done, step, obs_da)
        self._num_added += 1
    ...
    def _encode_sample(self, idxes):
        if self._da:
            obses_t, actions, rewards, obses_tp1, dones, steps, obs_da = [], [], [], [], [], [], []
            for i in idxes:
                data = self._storage[i]
                obs_t, action, reward, obs_tp1, done, step, o_da = data
            ...
            return (np.array(obses_t), np.array(actions), np.array(rewards),
                    np.array(obses_tp1), np.array(dones), np.array(steps),
                    np.array(obs_da))
        else:
            obses_t, actions, rewards, obses_tp1, steps, dones = [], [], [], [], [], []
            for i in idxes:
                data = self._storage[i]
                obs_t, action, reward, obs_tp1, done, step = data
                obses_t.append(np.array(obs_t, copy=False))
                actions.append(np.array(action, copy=False))
                rewards.append(reward)
                obses_tp1.append(np.array(obs_tp1, copy=False))
...
class PrioritizedReplayBuffer(ReplayBuffer):
    @DeveloperAPI
    def __init__(self, size, alpha, da):
        """Create Prioritized Replay buffer.
        ...
        """
        super(PrioritizedReplayBuffer, self).__init__(size, da)
        assert alpha > 0
        self._alpha = alpha
    ...
    @DeveloperAPI
    def add(self, obs_t, action, reward, obs_tp1, done, weight, steps, obs_da):
        """See ReplayBuffer.store_effect"""

        idx = self._next_idx
        super(PrioritizedReplayBuffer, self).add(obs_t, action, reward,
                                                 obs_tp1, done, weight, steps, obs_da)
        if weight is None:
    ...
    def __init__(self,
        ...
        def new_buffer():
            return PrioritizedReplayBuffer(
                self.buffer_size, alpha=prioritized_replay_alpha, da=da)
            ...
            for policy_id, s in batch.policy_batches.items():
                for row in s.rows():
                    self.replay_buffers[policy_id].add(
                        ...
                        if "weights" in row else None, row['steps'], 
                        row['obs_da'] if 'obs_da' in row else None)
    ...
    def replay(self):
        ...
        with self.replay_timer:
            ...
            for policy_id, replay_buffer in self.replay_buffers.items():
                ...
                data = replay_buffer.sample_with_idxes(
                     idxes, beta=self.prioritized_replay_beta)
                if len(data) == 8:
                    (obses_t, actions, rewards, obses_tp1, dones, steps, weights,
                        batch_indexes) = data
                    ...
                    (obses_t, actions, rewards, obses_tp1, dones, steps, obs_da, weights,
                        batch_indexes) = data
                    samples[policy_id] = SampleBatch({
                        "obs": obses_t,
                        "actions": actions,
